...
const handleMatchFound = (data) => {
  status.value = 'found'
  // Store race data in session/state if needed
  // Redirect to the actual race page (we will build this next)
  setTimeout(() => {
    router.push({
      name: 'singleplayer', // Temporary until we create 1v1 view
      query: { 
        race: data.race_uuid, 
        mode: 'multi', 
        challenge: data.challenge_uuid,
